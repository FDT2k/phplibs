...
class DatabaseSQLite extends Database{
	...
	protected function connect($database,$options){
		$this->error = ERROR_NO_ERROR;
		Env::getLogger()->log('Connecting to '.$user.'@'.$server.'/'.$db.' :'.$this->getOption('forceNewLink') );
		if($this->handler = mysqli_connect($server,$user,$password,$this->getOption('forceNewLink'))){
...
//var_dump($this->cache);
	...
	public function getFieldName($num) {
		$this->error=ERROR_NO_ERROR;
		$fieldInfo=mysqli_fetch_field($this->resultset, $num);
	...
	public function fetchField($num) {
		$this->error=ERROR_NO_ERROR;
		return mysqli_fetch_field($this->resultset, $num);
	}
